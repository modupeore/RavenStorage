...
customSNPanalysisPipeline.pl --in1 fastqfolder/ --in2 outputfolder/ [--help] [--manual]
...
=item B<-3, --in3, --list>=NUMBERS

Specify libraries interested in computing, if more than one
	separate them by the comma (",") sign (e.g 223,224,225).  (Required)
...
#ARGUMENTS
my($fastqfolder,$destfolder,$libraries,$help,$manual);
...
#library list
my @list = split("\,", $libraries); #library numbers
...
###WORKING
#processing custom list of fastqfiles
foreach my $libraryno (@list){
	my $raw_reads = `find $fastqfolder -name '$libraryno\_*gz'`; 
