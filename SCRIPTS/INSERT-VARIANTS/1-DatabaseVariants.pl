...
databaseVariants.pl --in FOLDER [--help] [--manual]
...
# CREATING LOG FILES
my $std_out = '/home/modupe/.LOG/variants-'.`date +%m-%d-%y_%T`; chomp $std_out; $std_out = $std_out.'.log';
my $std_err = '/home/modupe/.LOG/variants-'.`date +%m-%d-%y_%T`; chomp $std_err; $std_err = $std_err.'.err';
...
print NOTE "Subject: Running databaseVariants.pl!!!\n\nName of files\n\t$std_out\n\t$std_err\n"; 
...
$syntax = "select library_id from VARIANT_FILES where status is NULL";
...
for (my $i= 1; $i < $count+1; $i++){
	...
	#DELETE FROM VARIANT_SUMMARY
	$sth = $dbh->prepare("delete from VARIANT_SUMMARY where library_id = ?"); 
	$sth->execute( $colzz ); 
	#DELETE FROM VARIANT_RESULTS
	$sth = $dbh->prepare("delete from VARIANT_RESULTS where library_id = ?"); 
	$sth->execute( $colzz );
	#DELETE FROM VARIANT_FILES
	$sth = $dbh->prepare("delete from VARIANT_FILES where library_id = ?"); 
	$sth->execute( $colzz );
}
#CHECKING THE LIBRARIES ALREADY IN THE DATABASE
$syntax = "select library_id from VARIANT_FILES";
...
foreach my $library (@Directory){
	#CHECKING THE LIBRARIES ALREADY IN THE DATABASE
	my $number = 0; my %Hashresults = undef;
	$syntax = "select library_id from VARIANT_FILES";
	$sth = $dbh->prepare($syntax); $sth->execute or die "SQL Error: $DBI::errstr\n";
	while ($row = $sth->fetchrow_array() ) {
		$Hashresults{$row} = $number; $number++;
	...
	if ($library =~ /^library-(\d*)$/) { # parsing library name 
		my $libnumber = "$1$code";
		unless (exists $Hashresults{$libnumber}){
			print "\n. . .\n\tWORKING ON $library\n\n";
