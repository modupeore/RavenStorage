...
while ($addi<$sum) {
  ...
  foreach my $genename (sort keys %GENE ){
    ...
    foreach my $number (1..$GENE{$genename}){
      ...
      my $runkaks = "$KAKS -i $fileloc.axt -o $fileloc.kaks -m NG -m LWL -m LPB -m MLWL -m MLPB -m GY -m YN -m MYN -m MS -m MA -m GNG -m GLWL -m GLPB -m GMLWL -m GMLPB -m GYN -m GMYN";
